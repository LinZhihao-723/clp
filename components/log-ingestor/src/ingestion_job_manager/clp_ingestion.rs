...
impl ClpCompressionState {
    ...
    pub async fn submit_for_compression(
        ...
        const SET_READ_COMMITTED_QUERY: &str = "SET TRANSACTION ISOLATION LEVEL READ COMMITTED";
        ...
        // NOTE: `SET TRANSACTION ISOLATION LEVEL READ COMMITTED` (without `SESSION`/`GLOBAL`) applies
        // only to the very next transaction started on this connection, so the isolation level is
        // scoped to this submission and the pooled connection reverts to the server default
        // afterwards. Using `SESSION` would leak the level to other queries that reuse the pooled
        // connection. READ COMMITTED avoids the gap/next-key locking that made the batched status
        // UPDATE below contend heavily under concurrency (see docs/2026-07-05-50-job-db-contention.md).
        let mut conn = self.db_pool.acquire().await?;
        sqlx::query(SET_READ_COMMITTED_QUERY)
            .execute(&mut *conn)
            .await?;
